        ...
        struct instance_culling_of_stratholme_InstanceMapScript : public InstanceScript
        {
            ...
            void FillInitialWorldStates(WorldPacket& packet) override
            {
                for (WorldStateMap::const_iterator itr = _sentWorldStates.begin(); itr != _sentWorldStates.end(); ++itr)
                    packet << uint32(itr->first) << uint32(itr->second);
            }
            ...
            void ReadSaveDataMore(std::istringstream& data) override
            {
                ...
                time_t timediff = (infiniteGuardianTime - GameTime::GetGameTime().count());
                if (!infiniteGuardianTime)
                    timediff = -1;

                LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::ReadSaveDataMore: Loaded with state {} and guardian timeout at {} minutes {} seconds from now", (uint32)loadState, timediff / MINUTE, timediff % MINUTE);
            }
            ...
            void SetGuidData(uint32 type, ObjectGuid guid) override
            {
                switch (type)
                {
                    ...
                    case DATA_UTHER_START:
                        LOG_DEBUG("scripts.cos", " ?? DATA_UTHER_START");
                        if (_currentState == CRATES_DONE)
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {                         // we do the whisper as a guardian tick because i don't want to duplicate the real-time code
                            ...
                            time_t secondsToGuardianDeath = _infiniteGuardianTimeout - GameTime::GetGameTime().count();
            ...
            void OnCreatureCreate(Creature* creature) override
            {
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_CRATE_HELPER:
                        ...
                        break;
                    case NPC_ARTHAS:
                        LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::OnCreatureCreate: Arthas spawned at {}", creature->GetPosition().ToString());
                        _arthasGUID = creature->GetGUID();
                        creature->setActive(true);
                        break;
                    case 26528:
                        LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::OnCreatureCreate: NPC_UTHER spawned at {}", creature->GetPosition().ToString());
                        _arthasGUID = creature->GetGUID();
                        creature->setActive(true);
                        break;
                    case 26497:
                        LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::OnCreatureCreate: NPC_JAINA spawned at {}", creature->GetPosition().ToString());
                        _arthasGUID = creature->GetGUID();
                        creature->setActive(true);
                        break;
            ...
            void SetInstanceProgress(COSProgressStates state, bool force)
            {
                ...
                switch (state)
                {
                    case CRATES_DONE:
                        ...
                        if (Creature* bunny = instance->GetCreature(_genericBunnyGUID)) {
                            ...
                            LOG_DEBUG("scripts.cos", " >> YES BUNNY");
                        }
                        else {
                            LOG_DEBUG("scripts.cos", " >> NO BUNNY");
                        }
                ...
                if (force)
                {
                    ...
                    for (std::unordered_map<ObjectGuid, Creature*>::const_iterator itr = objects.cbegin(); itr != objects.cend(); ++itr)
                    {
                        ...
                        {
                            if (itr->second->getDeathState() == DeathState::Dead) // despawned, not corpse
        ...
        private:
            ...
            void SpawnInfiniteCorruptor()
            {
                if (!_infiniteGuardianTimeout && instance->GetSpawnMode() == DUNGEON_DIFFICULTY_HEROIC && (GetBossState(DATA_INFINITE_CORRUPTOR) != DONE && GetBossState(DATA_INFINITE_CORRUPTOR) != FAIL))
                {
                    ...
                    _infiniteGuardianTimeout = GameTime::GetGameTime().count() + 25 * MINUTE;
            ...
            {
                LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::SetWorldState: {} {}", uint32(state), value);
            ...
            void PropagateWorldStateUpdate()
            {
                ...
                for (WorldStateMap::const_iterator it = _currentWorldStates.begin(); it != _currentWorldStates.end(); ++it)
                {
                    ...
                    if (sent != it->second)
                    {
                        LOG_DEBUG("scripts.cos", "instance_culling_of_stratholme::PropagateWorldStateUpdate: Sending world state {} ({})", it->first, it->second);
