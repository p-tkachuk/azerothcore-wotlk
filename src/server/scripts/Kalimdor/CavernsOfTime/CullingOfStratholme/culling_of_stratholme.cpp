                    ...
                    case GOSSIP_OFFSET_OPEN_GM_MENU:
                        ...
                        for (uint32 state = 1; state <= COMPLETE; state = state << 1)
                        {
                            if (GetStableStateFor(COSProgressStates(state)) == state)
                                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, Acore::StringFormat("Set instance progress to 0x{:05X}", state), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + AsUnderlyingType(GOSSIP_OFFSET_GM_INITIAL) + state);
                        }
                        for (uint32 state = 1; state <= COMPLETE; state = state << 1)
                        {
                            if (GetStableStateFor(COSProgressStates(state)) != state)
                                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, Acore::StringFormat("Force state to 0x{:05X} (UNSTABLE)", state), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + AsUnderlyingType(GOSSIP_OFFSET_GM_INITIAL) + state);
                        }
                        SendGossipMenuFor(player, GOSSIP_TEXT_SKIP_1, me->GetGUID());
                        break;
...
class npc_chromie_middle : public CreatureScript
{
    public:
        ...
        struct npc_chromie_middleAI : public NullCreatureAI
        {
            ...
            void MoveInLineOfSight(Unit* unit) override
            {
                LOG_TRACE("scripts.cos", " ??????????? MoveInLineOfSight");
            ...
            if (type == SPLINE_CHAIN_MOTION_TYPE)
            {
                switch (id)
                {
                    case MOVEID_EVENT1:
                        ...
                        me->SetFacingTo(marthaIdleOrientation1);
                        Events.ScheduleEvent(EVENT_MARTHA_IDLE2, Seconds(9), Seconds(15));
                        break;
                    case MOVEID_EVENT2:
                        me->SetEmoteState(EMOTE_STATE_USE_STANDING);
                        me->SetFacingTo(marthaIdleOrientation2);
