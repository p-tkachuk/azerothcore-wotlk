    ...
    struct npc_arthas_stratholmeAI : public ScriptedAI
    {
        ...
        void AdvanceToState(COSProgressStates newState)
        {
            ...
            if (itr != ArthasSnapbackPositions.end())
            {
                ...
                LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: has snapback for this state, distance = {}", target.SnapbackPosition->GetExactDist(me));
                ...
                if (target.SnapbackPosition->GetExactDist(me) > ArthasSnapbackDistanceThreshold)
                    me->NearTeleportTo(target.SnapbackPosition->GetPositionX(), target.SnapbackPosition->GetPositionY(), target.SnapbackPosition->GetPositionZ(), 0);
            }
            ...
            switch (newState)
            {
                ...
                case UTHER_TALK:
                    ...
                    {
                        LOG_DEBUG("scripts.cos", " YES!!!!! 222222");
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            switch (id)
            {
                ...
                case RP5_POINTID_ARTHAS4:
                    me->NearTeleportTo(
                        ArthasPositions[ARTHAS_FINAL_POS].GetPositionX(),
                        ArthasPositions[ARTHAS_FINAL_POS].GetPositionY(),
                        ArthasPositions[ARTHAS_FINAL_POS].GetPositionZ(), 0
                    );
                    break;
                default:
                    break;
                ...
                switch (event)
                {
                    ...
                    case RP2_EVENT_REACT5:
                    {
                        ...
                        if (!nearbyVictims.empty())
                        {
                            ...
                            Creature* victim = Acore::Containers::SelectRandomContainerElement(nearbyVictims);
                            if (victim->IsAlive())
                                victim->HandleEmoteCommand(Acore::Containers::SelectRandomContainerElement(emotes));
                        }
                        break;
                    }
                    ...
                    case RP3_EVENT_SPAWN1:
                    {
                        ...
                        if (Creature* rift = instance->instance->SummonCreature(NPC_TIME_RIFT, ArthasPositions[RP3_SPAWN1_RIFT]))
                            rift->DespawnOrUnsummon(4 * IN_MILLISECONDS);
                        break;
                    }
                    ...
                    case RP3_EVENT_EPOCH_SPAWN:
                        ...
                        if (Creature* rift = instance->instance->SummonCreature(NPC_TIME_RIFT_LARGE, ArthasPositions[RP3_EPOCH_RIFT]))
                            rift->DespawnOrUnsummon(27 * IN_MILLISECONDS);
                        break;
        ...
        {
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: RP in progress? '{}'", _progressRP ? "YES" : "NO");
            if (_progressRP)
            {
                _progressRP = false;
        ...
        {
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::EnterEvadeMode: why = {} ", why);
        ...
        void sGossipSelect(Player* player, uint32 /*sender*/, uint32 listId) override
        {
            uint32 const action = GetGossipActionFor(player, listId);
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::GossipSelect: '{}' selects action '{}' on '{}'", player->GetGUID().ToString(), action, me->GetGUID().ToString());
